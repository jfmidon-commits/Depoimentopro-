...
function fail(res, status, message) {
  ...
  return res.end(`<p>${esc(message)}</p>`);
}
...
module.exports = async (req, res) => {
  ...
  if (!/^[A-Za-z0-9_-]{24,128}$/.test(token)) {
    return fail(res, 400, 'Widget inválido.');
  }

  try {
    const safeToken = air.escapeFormula(token);
    const widget = await air.findOne(
      TABLES.widgets,
      `AND({Public Token}='${safeToken}',{Ativo}=1)`
    );
    if (!widget) return fail(res, 404, 'Widget não encontrado ou desativado.');

    const userId = widget.fields?.['User']?.[0];
    if (!/^rec[A-Za-z0-9]{14}$/.test(String(userId || ''))) {
      return fail(res, 400, 'Widget inválido.');
    }

    const requestedLimit = Number(widget.fields?.['Limite'] || 5);
    const widgetLimit = Math.max(1, Math.min(20, requestedLimit));
