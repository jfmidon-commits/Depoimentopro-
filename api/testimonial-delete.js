...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const rate = await checkRateLimit({ ...LIMITS.moderationUser, identity: session.userId });
    setRateLimitHeaders(res, rate);
    if (!rate.allowed) return rejectRateLimit(res, rate, 'Muitas ações em pouco tempo. Aguarde um instante.');
    ...
    const updates = [
      air.update(TABLES.users, session.userId, { 'Depoimentos Usados': userTestimonials.length }),
    ];
    ...
    if (error?.statusCode === 404) return json(res, 404, { error: 'Depoimento não encontrado.' });
    logError('testimonial_delete.failed', error, { requestId, route: '/api/testimonial-delete' });
