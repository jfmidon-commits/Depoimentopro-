...
async function updateSubscription(subscription, eventType) {
  if (!subscription) return { action: 'ignored', reason: 'subscription-snapshot-unavailable' };
  ...
  const metadataUser = String(subscription?.metadata?.dpro_user_id || '').trim();
  if (metadataUser && metadataUser !== user.id) {
    log('warn', 'billing.subscription_metadata_user_mismatch', { eventType, userId: user.id });
    return { action: 'ignored', reason: 'metadata-user-mismatch' };
  }
...
async function latestSubscriptionSnapshot(subscription, eventType) {
  ...
  try {
    return await stripe.retrieveSubscription(subId);
  } catch (error) {
